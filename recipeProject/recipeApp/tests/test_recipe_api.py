...
RECIPES_URL = reverse('recipeApp:recipe-list')


def image_upload_url(recipe_id):
    """Return URL for recipe image upload"""
    return reverse('recipeApp:recipe-upload-image', args=[recipe_id])
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_partial_update_recipe(self):
        """Test updating a recipe with patch --> change a column"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
    ...
    def test_full_update_recipe(self):
        """Test updating a recipe with put-> delte all the data and insert all of them again, if you dont fill a field this one would be blank"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
