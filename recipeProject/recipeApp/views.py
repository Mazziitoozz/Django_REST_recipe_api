...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    @action(methods=['POST'], detail=True, url_path='recipe-upload-image') # we said the method and the url with the action
    def upload_image(self, request, pk=None):
        ...
        if serializer.is_valid(): # to avaoid receive wrong data we need to check that they are correct
            serializer.save()
            return Response(
                serializer.data,
                status=status.HTTP_200_OK
            )
        # if they are incorrect return error
        return Response(
            serializer.errors,
